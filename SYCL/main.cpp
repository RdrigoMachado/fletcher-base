...
int main(int argc, char** argv) {
    ...
    //SYCL initialization of Device and Queue
    auto devices = sycl::device::get_devices(sycl::info::device_type::gpu);
    if(devices.size() == 0){
        printf("Not enogh devices. Terminating\n");
        return 0;
    }
    device = devices[0];
    q = sycl::queue(device);
    ...
    float* vpz      = sycl::malloc_host<float>(sx*sy*sz, q);    // p wave speed normal to the simetry plane
    float* vsv      = sycl::malloc_host<float>(sx*sy*sz, q);    // sv wave speed normal to the simetry plane
    float* epsilon  = sycl::malloc_host<float>(sx*sy*sz, q);    // Thomsen isotropic parameter
    float* delta    = sycl::malloc_host<float>(sx*sy*sz, q);    // Thomsen isotropic parameter
    float* phi      = sycl::malloc_host<float>(sx*sy*sz, q);    // isotropy simetry azimuth angle
    float* theta    = sycl::malloc_host<float>(sx*sy*sz, q);    // isotropy simetry azimuth angle
    ...
    float* pp = sycl::malloc_host<float>(sx*sy*sz, q);
    float* pc = sycl::malloc_host<float>(sx*sy*sz, q);
    float* qp = sycl::malloc_host<float>(sx*sy*sz, q);
    float* qc = sycl::malloc_host<float>(sx*sy*sz, q);
