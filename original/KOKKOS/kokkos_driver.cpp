...
void DRIVER_Initialize(const int sx, const int sy, const int sz, const int bord,
                       ...
                       HostViewFloat1D phi, HostViewFloat1D theta,
                       HostViewFloat1D ch1dxx, HostViewFloat1D ch1dyy, HostViewFloat1D ch1dzz, HostViewFloat1D ch1dxy,
                       HostViewFloat1D ch1dyz, HostViewFloat1D ch1dxz, HostViewFloat1D v2px, HostViewFloat1D v2pz, HostViewFloat1D v2sz, HostViewFloat1D v2pn,
                       ...
                       DeviceViewFloat1D dev_qp, DeviceViewFloat1D dev_qc)
{
    ...
    ch1dxx = Kokkos::View("ch1dxx", msize_vol);
    ch1dyy = Kokkos::View("ch1dyy", msize_vol);
    ch1dzz = Kokkos::View("ch1dzz", msize_vol);
    ch1dxy = Kokkos::View("ch1dxy", msize_vol);
    ch1dyz = Kokkos::View("ch1dyz", msize_vol);
    ch1dxz = Kokkos::View("ch1dxz", msize_vol);
...
ch1dxx(iPrint), ch1dyy(iPrint), ch1dzz(iPrint), ch1dxy(iPrint), ch1dxz(iPrint), ch1dyz(iPrint));
    ...
    v2px = Kokkos::View("v2px", msize_vol);
    v2pz = Kokkos::View("v2pz", msize_vol);
    v2sz = Kokkos::View("v2sz", msize_vol);
    v2pn = Kokkos::View("v2pn", msize_vol);
    for (int i=0; i<msize_vol; i++){
        v2sz(i)=vsv(i)*vsv(i);
        v2pz(i)=vpz(i)*vpz(i);
